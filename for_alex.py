...
def main():
    # base_path = pathlib.Path(file).parent.joinpath('new_files')
    ...
    try:
        files = [unpack_file_zip('Huawei_RRL_Capacity_4pika_pla'),
                 unpack_file_zip('Ericsson_RRL_Capacity_4pika'),
                 unpack_file_zip('NEC_RRL_Capacity_4pika')]
    except:
        files = [unpack_file_any(get_path_hw('Huawei_RRL_Capacity_4pika_pla')),
                 unpack_file_any(get_path_hw('Ericsson_RRL_Capacity_4pika')),
                 unpack_file_any(get_path_hw('NEC_RRL_Capacity_4pika'))]
    ...
    for year in split_by_years(df_with_type_identification):
        ...
        dataframes.append(summary_by_year_HW)
