...
def df_type_identification(dataframe):
    ...
    for index, row in deepcopy(dataframe).iterrows():
        ...
        if data_row['FULL_CAPACITY'] <= 495 and str(data_row['xpiccalculated']) == 'nan' and data_row[
            'BaseTXFrequency'] < 70000:
            data_row['Type'] = '1+0'
        elif str(data_row['xpiccalculated']) != 'nan':
            data_row['Type'] = 'XPIC/2+0'
            # print(type(data_row['BaseTXFrequency']))
        elif data_row['BaseTXFrequency'] >= 70000:
                        ...
                        if data_support_row['Type'] == '1+0':
                            if formula < 350:
                                data_support_row['Extension'] = 'SW'
                                    ...
                                    and temp_row['Maplength_m_'] <= 3500:
...
def unpack_file_zip(file_name):
    """ Распаковка .zip """

    path = get_path_hw(file_name)
...
def unpack_file_any(path):
    ...
    # dir_out = r"E:\Python"
    dir_out = r"L:\Transport_planning\VISIO ЧТП\Access\Operation Group\Данные по статистике\Weekly_RRL_Integrity"
